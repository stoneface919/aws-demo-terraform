...
resource "aws_subnet" "app02-subnet" {
    vpc_id = aws_vpc.app02_vpc.id
    tags = {
...
# VIRTUAL ROUTER 
resource "aws_route_table" "app02-route_table" {
    vpc_id = aws_vpc.app02_vpc.id
    route {
        # IG
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.app02-internet_gateway.id
...
resource "aws_internet_gateway" "app02-internet_gateway" {
    vpc_id = aws_vpc.app02_vpc.id
    tags = {
...
resource "aws_route_table_association" "app02-rtb-subnet" {
    subnet_id = aws_subnet.app02-subnet.id
    route_table_id = aws_route_table.app02-route_table.id
}

# CREATE A SECURITY GROUP - CONFIGURE INBOUND/OUTBOUND RULES
resource "aws_security_group" "app02-sg" {
    name = "app02-sg"
...
resource "aws_instance" "app02-server" {
    ami = data.aws_ami.latest-amazon-linux-image.id
    instance_type = var.instance_type
    vpc_security_group_ids = [aws_security_group.app02-sg.id]
...
# CREATE KEY PAIR
resource "aws_key_pair" "app02-keypair" {
  key_name = "app02-server-key"
